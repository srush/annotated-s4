...
class TestLayer(nn.Module):
    ...
    def setup(self):
        self.B = self.param("B", normal(dtype=np.complex64), (self.N,))
        self.C = self.param("C", normal(dtype=np.complex64), (self.N,))
        # self.B = self.param("B", normal(), (self.N, 2))
        # self.B = self.B[..., 0] + 1j * self.B[..., 1]
        # self.C = self.param("C", normal(), (self.N, 2))
        # self.C = self.C[..., 0] + 1j * self.C[..., 1]

        self.K = cauchy(np.arange(self.l_max), self.B*self.C).real

    def __call__(self, u):
        return u + self.K


TestLayer = cloneLayer(TestLayer)
