...
from jax.numpy.linalg import eig, eigvals, inv
...
def discretizeSSM_bilinear(ssm, step):
    I = np.eye(ssm.N)
    left = inv((I - (step / 2.0) * ssm.A))
    Abar = left @ (I + (step / 2.0) * ssm.A)
...
def K_conv_naive(ssm, L):
    assert ssm.discrete
    return np.array([(ssm.C @ power(ssm.A, l) @ ssm.B).reshape()[0] for l in range(L)])
...
class S4:
    def __init__(self, L, N, step=1):
        ...
        self.C = Param((1, N))
        self.A = (
            np.diag(self.Gamma) - self.p[:, np.newaxis] * self.q[np.newaxis, :].conj()
        )
        ...
        self.Ct = (
            np.eye(N) - power(self.discrete.A, self.L)
        ).conj().T @ self.discrete.C.ravel()
        # self.Ct = self.discrete.C @ (np.eye(N) - power(self.discrete.A, self.L))

    def K_gen(self):
        return ssmGeneratingFn(
            (self.Ct.conj().ravel(), self.q.conj().ravel()),
